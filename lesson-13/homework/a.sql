...
select*from Orders
select p.product_name,min (o.unit),o.order_date
from Orders as o
join Products  as p on p.product_id=o.product_id
where MONTH (o.order_date)= 'February' and YEAR (o.order_date)='2020'
group by  p.product_name,o.unit,o.order_date
...
-- Íàõîäèì ïåðâûé è ïîñëåäíèé ID
SELECT @CurrentOrderID = MIN(OrderID), 
       @MaxOrderID = MAX(OrderID)
FROM Orders;

-- Ïîêà íå äîøëè äî ïîñëåäíåãî çàêàçà
...
BEGIN
    ...
    PRINT CONCAT('Çàêàç ¹', @CurrentOrderID, ' — ñóììà: ', @Amount);

    SET @CurrentOrderID = @CurrentOrderID + 1; -- èä¸ì ê ñëåäóþùåìó çàêàçó
END;
...
--'The data was cleaner than my kitchen before guests arrive, which is statistically rare'
--shu gapdagi A harfi nechta ekanligini topish lozim
--While loop hamda Recursive cte yordamida 
declare @sentence varchar(100) = 'The data was cleaner than my kitchen before guests arrive, which is statistically rare',
@checker int = 1,
@counter_a int = 0

while @checker <=len(@sentence)
  begin
    if SUBSTRING(@sentence,@checker,1) = 'a'
      begin
...
CAST(123.45 AS INT)   -- ðåçóëüòàò: 123
