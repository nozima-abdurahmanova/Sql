...
--2. Find strings with % (TestPercent)
...
6. Count spaces in string (CountSpaces)

SELECT 
    LEN(col) - LEN(REPLACE(col, ' ', '')) AS SpaceCount
FROM CountSpaces;


--7. Employees earning more than their managers (Employee)

SELECT e.EmployeeID, e.Name, e.Salary, m.Name AS Manager, m.Salary AS ManagerSalary
FROM Employee e
...
--1. Separate integers and characters (rtcfvty34redt)

SELECT 
    col,
    LEFT(col, PATINDEX('%[0-9]%', col)-1) AS Letters,
    SUBSTRING(col, PATINDEX('%[0-9]%', col), LEN(col)) AS Numbers
FROM YourTable;


--2. Higher temperature than previous day (weather)
...
3. First login date for each player (Activity)
...
--4. Return third item from list (fruits)

SELECT value
FROM STRING_SPLIT((SELECT TOP 1 col FROM fruits), ',')
ORDER BY (SELECT NULL)
OFFSET 2 ROWS FETCH NEXT 1 ROWS ONLY;


--5. Convert string into rows (sdgfhsdgfhs@121313131)
...
--6. Join p1 & p2, replace code if 0
...
--8. Extract integer at start of string (GetIntegers)

SELECT 
    LEFT(Vals, PATINDEX('%[^0-9]%', Vals)-1) AS StartingNumber
FROM GetIntegers;

--🔴 Difficult Tasks

--1. Swap first two letters of each comma-separated value (MultipleVals)

SELECT 
    STRING_AGG(
        STUFF(STUFF(value, 1, 1, RIGHT(value, 1)), 2, 1, LEFT(value, 1)), ','
    ) AS SwappedValues
FROM STRING_SPLIT((SELECT TOP 1 col FROM MultipleVals), ',');


2. First device logged in per player (Activity)
...
--3. Week-on-week percentage of sales per area (WeekPercentagePuzzle)
...
daily AS (
    ...
    FROM sales
    GROUP BY area, DATEPART(week, sale_date), sale_date
),
total_week AS (
    SELECT week_num, SUM(weekly_sales) AS total_sales
    FROM weekly
    GROUP BY week_num
)
SELECT 
