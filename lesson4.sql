...
SELECT c.CustomerName, p.ProductName, s.SaleAmount
FROM Sales s
...
SELECT TOP 3 ProductID, COUNT(*) AS SaleCount
FROM Sales
GROUP BY ProductID
ORDER BY SaleCount DESC;

SELECT TOP 1 CustomerID, SUM(SaleAmount) AS TotalSpent
FROM Sales
GROUP BY CustomerID
ORDER BY TotalSpent DESC;

SELECT s.SaleID, s.SaleAmount, p.Price
FROM Sales s
JOIN Products p ON s.ProductID = p.ProductID
WHERE s.SaleAmount > p.Price;
